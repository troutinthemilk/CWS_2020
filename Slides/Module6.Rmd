...
Slides: https://troutinthemilk.github.io/CWS_2020/Slides/Module6.html

Exercises and groupwork: https://rstudio.cloud/spaces/73676/project/1350152

Code to reproduce slides: https://github.com/troutinthemilk/CWS_2020/blob/master/Slides/Module6.Rmd
...
**Multicollinearity** - when more than two predictor variables are correlated with each other.
...
- Variables that are collinear and each have their own separate “effect” on the response variable
...
Consider goals: if **prediction** is your goal then collinearity may not be a problem.
...
.footnote[
image source: http://jarrettbyrnes.info

Online textbook: [Structural Equation Modeling in R for Ecology and Evolution](https://jslefche.github.io/sem_book/), Lefcheck, J.
]
...
Note that different criterion will often point to different models as best!
...
```{r}
train.dat <- sleep.dat[1:30,]
test.dat <- sleep.dat[31:41,]
...
3. Calculate the error between the predicted and held-out value, $(\hat{Y}- Y)^2$.
...
* Fit all models, then the lowest AIC (AICc) value is the best model.
...
```{r, warning=F, message=F}
library(MuMIn)
model.sel(list(mod2, mod3), rank=AICc)
```

<br>

AICc selects model 2 but $\Delta$AIC = 2.57.
...
model.sel(list(mod1, mod2, mod3, mod4), rank=AICc)
```
...
```{r, warning=F, message=F}
AIC.list <- model.sel(list(mod1, mod2, mod3, mod4), rank=AICc)

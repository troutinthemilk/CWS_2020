...
```{r, out.width = "68%", eval=T, cache=FALSE, fig.asp = 0.7, warning=F}
...
```{r, out.width = "80%", fig.asp = 0.7, warning=F}
ggplot(data=mpg) + 
...
```{r, out.width = "70%", fig.asp = 0.7, warning=F}
ggplot(data=mpg) + 
...
```{r, out.width = "60%", fig.asp = 0.7, warning=T}
ggplot(data=mpg) + 
...
ggplot(mpg, aes(x=displ, y=hwy, color=class)) + 
  geom_point(aes(size=year), alpha=0.5) 
```
...
```{r out.width = "80%", fig.asp = 0.7, fig.width=8, warning=F}
ggplot(mpg, aes(x=displ, y=hwy, color=class)) + 
...
```{r out.width = "70%", fig.asp = 0.7, fig.width=8, warning=F}
...
# Test other geometries: `geom_smooth`

```{r out.width = "70%", fig.asp = 0.7, warning=F, message=F}
ggplot(iris) + 
...
```{r out.width = "60%", fig.asp = 0.7, warning=F, message=F}
...
```{r out.width = "75%", fig.asp = 0.7, warning=F, message=F}
ggplot(iris, aes(Petal.Length, Petal.Width)) + 
...
# Use a linear model instead of a smooth

```{r out.width = "75%", fig.asp = 0.7, message=F}
ggplot(iris, aes(Petal.Length, Petal.Width, color = Species)) + 
   geom_point(aes(shape = Species), size = 3) +
  geom_smooth(aes(group = Species), color="black", method="lm") 
...
# Stacked bar charts for multiple categorical variables
...
---
# Proportions
Use `position="fill"` to make proportions
...
p <- ggplot(iris, aes(Petal.Length, Petal.Width, color = Species, shape = Species)) + geom_point(size = 3)
```

...
>
> `r tufte::quote_footer('--- Hadley Wickham (2010)')`
...
knitr::include_graphics("../figure/PackageMenu.png")
```
...
```{r, out.width = "50%", eval=T, cache=FALSE, fig.asp = 0.7, warning=F, fig.align="center"}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x=displ, y=hwy))
```
...
```{r out.width = "65%", fig.asp = 0.7, fig.width=8, warning=F, fig.align="center"}
ggplot(data=mpg) + 

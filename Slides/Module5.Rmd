...
Sleep is characterized by either slow wave (non-dreaming) or drapid eye movement (dreaming), with wide variability in theamount of both types of sleep.
...
```{r, echo=F, fig.align="center", out.width="70%", fig.asp=0.6}
...
```{r}
cor(sleep.dat$BrainWt, sleep.dat$TotalSleep, use="pairwise.complete") #the argument to use, removes data that is missing. 
```
---
...
fits a line to the data. Estimates the **intercept** and **slope**. 
...
```{r, fig.aling="center", fig.asp=0.8}
...
```{r, out.width="70%", fig.align="center", fig.asp=0.7}
library(sjPlot)
...
I the previous model the intercept varied by auction house. In this model **both** the intercept and slope vary.
...
- Fit continuous response data to one or more predictors using 

...
Histograms are univariate plots (`geom_histogram`) used to illustrate numerical variables and barcharts (`geom_bar`) are univariate plots used to illustrate categorical variables. In both of these kinds of plots we only specify the aesthetic `x` (no `y` aesthetic). 

1. Build a histogram of Elk counts in Yellowstone National Park using the variable `Elk`.
2. Build a barplot from the Titanic dataset using the `embarked` variable (locations that passengers embarked from). Bonus points if you can figure out how to also get survival on the embarked plot.
